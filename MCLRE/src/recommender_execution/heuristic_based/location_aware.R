...
GeoProfileRecommender <- function(user, event.cand.test, h){
  """
    user: | test user_id | longitude | latitude |
    event.cand.test: | test event_id | longitude | latitude | time |
    h: probably an integer
  """
  ...
  if (nrow(events.train) > 0){
    ...
    # [, 2:3] selects columns 2 AND 3 (long and lat)
