...
if __name__ == '__main__':
    ...
    #   (mrbpr_relation_names, partitions)
    #   or (bpr-net_relation_rames, partitions)
    # and values:
    #   (see: experiment_atts.csv)
    ...
    # TRAIN_RELATION_NAMES: something like user-event-rsvp, user-group-participate, group-event-created
    #                                      user-user-same-group, user-user-same-event
    TRAIN_RELATION_FILES = ["%s_train.tsv" % name for name in TRAIN_RELATION_NAMES]
    PARTITIONS = reversed(EXPERIMENT_ATTS['partitions']) # iterator that accesses the sequence in
                                                          # reverse order
