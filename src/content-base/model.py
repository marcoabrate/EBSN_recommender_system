...
class EventContentModel(object):
    ...
    def __init__(self, pre_processment, algorithm, hyper_parameters):
    ...
    def read_and_pre_process_corpus(self, filename, content_columns, no_below_freq=0):
        """ Read and preprocess the corpus
            Args:
                filename: location of the csv file containing event ids and name, description
                content_columns: column names for creating the corpus ("name", "description", ...)
                no_below_freq: (optional) discard words under this frequency
            Return:
                Dictionary: event id -> selected columns content (e.g.
        ...
        with open(filename, "r") as csv_file:
            ...
            for row in csv_reader:
                ...
                # create dictionary from index to event id and from event id to index
                ...
                # extract the Words from the Event name and description
                event_words = self.extract_content(dict_event_content[event_id], content_columns)

                # update the BOW dictionary
    ...
    def post_process_corpus(self, post_process_types, params, dict_event_content, content_columns):
        """ Apply the Corpus post processments
            Args:
                post_process_type: can be ["filter_extreme_words"], or empty
                params: "daily_decay" -> float
                dict_event_content: event id -> selected columns content (e.g.
                    Dictionary: name -> name of the event, description -> description of the event)
                content_columns: column names for creating the corpus ("name", "description", ...)
        """
        # Filter extreme words from dictionary (based on the Corpus Document Frequecy)
        ...
        for doc_index in sorted(self.dict_index_event_id.keys()):
            ...
            # extract the Words from the Event content columns
            event_words = self.extract_content(dict_event_content[event_id], content_columns)

            # Create the Event BOWs
            event_bow = self.dictionary.doc2bow(event_words, allow_update=False)

            # Store the BOW in memory, one BOW for each event
    ...
    def train_model(self):
        ...
        if self.algorithm == "TFIDF":
            ...
            self.model = models.LsiModel(transformed_corpus,
                # ^Chaining: LSI (TFIDF or NOT (BOW))
                                         num_topics=self.hyper_parameters['num_topics'],
                                         id2word=self.dictionary,
                                         chunksize=20000,
                # ^BIGGER = higher speed | SMALLER = lower memory footprint (default)
                                         decay=1.0,
                # ^Decay to old documents, less than 1.0
                # gives less emphasis to old observations (default)
                                         distributed=False,
                # ^Single machine is enough (default)
                                         onepass=True,
                # ^One pass over data = faster (default)
                                         power_iters=5,
                # ^More Power Iterations to improve accuracy
                                         extra_samples=None)
                # ^None, so it will be dinamically defined (i.e. 2 * num_topics)
            ...
            self.model = models.LdaModel(transformed_corpus,
                # ^Chaining: LDA (TFIDF or NOT(BOW))
                                         num_topics=self.hyper_parameters['num_topics'],
                                         id2word=self.dictionary,
                                         distributed=False,
                # ^Single machine is enough (default: False)
                                         chunksize=2000,
                # ^Number of documents per inference cicle (default: 2000)
                                         passes=self.hyper_parameters['num_corpus_passes'],
                # ^Passes over the Full Corpus, increased to improve accuracy (default: 1)
                                         update_every=1,
                # ^Update the model every 1 document chunk (default: 1)
                                         alpha='auto',
                # ^Defines the gamma priors of the Dirichlet distributions
                ...
                #       (http://www.stanford.edu/~jhuang11/research/dirichlet/dirichlet.pdf)
                                         eta=None,
                # ^Defines the eta priors of the Dirichlet distributions
                #   The default (None) sets a symmetric prior (1.0/num_topics)
                # over the topic/word distribution
                                         decay=0.5,
                # ^Decay to old documents,
                #   decay=0.0 the new documents replace completely
                #      the old documents in every update
                #   decay=1.0 the new documents decrease its
                #       importance exponentially in every update
                #   (Hoffman et al. updates guarantees convergence in interval[0.5, 1)) (default: 0.5)
                                         eval_every=10,
                # ^Evaluate the Perplexity of the Model every N updates (default: 10)
                                         iterations=self.hyper_parameters['num_iterations'],
                # ^Number of Inference iterations over each chunk of documents (default: 50)
                                         gamma_threshold=0.00001)
                # ^Convergence Threshold, diff between two subsequent gamma values.
                #   Decreased to force all iterations (default: 0.001)


    def index_events(self, test_events):
    ...
    def query_model(self, query_model_format):
        """ Query the model given the query representation already in the model format
            Args:
                query_model_format: User Profile Representation
            Return:
        ...
        for index, similarity in index_similarities:
            event_id = candidate_event_ids[index]
            dict_event_score[event_id] = similarity
