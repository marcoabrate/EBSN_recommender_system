...
class BaseUserProfile(object):
    ...
    def __init__(self, profile_params, train_events_data, event_cb_model, dict_event_content):
        """ Constructor (Abstract Method)
            Args:
                profile_params: "daily_decay" -> float
                train_events_data:
...
class UserProfileSum(BaseUserProfile):
    ...
    def __init__(self, profile_params, train_events_data, event_cb_model, dict_event_content):
        ...
        for event_id in train_events_data['event_id_list']:
            train_event_bows.append(
                event_cb_model.corpus_of_bows[event_cb_model.dict_event_id_index[event_id]])

        # Sum all representations with EQUAL weights
        self._representation = self._transform_and_sum_event_bows(
            event_bows=train_event_bows,
...
class UserProfileTimeWeighted(BaseUserProfile):
    """ User Profile formed by the Weighted Sum of:
            ...
            (Weight based on the Time Distance from the RSVP to the Partition Time)
    """
    ...
    def __init__(self, profile_params, train_events_data, event_cb_model, dict_event_content):
        ...
        for rsvp_time in train_events_data['rsvp_time_list']:
            #days_from_rsvp = math.floor(
            #    (train_events_data['partition_time'] - rsvp_time)/float(60 * 60 * 24)) # Day in Sec
            days_from_rsvp = math.floor(
                 (rsvp_time)/float(60 * 60 * 24))
            event_weights.append(self._time_value_of_money(
                money_value=1,
    ...
    def _time_value_of_money(cls, money_value=1, time_step_decay=0, time_steps_in_the_future=0):
        """ Calculate the Time Value of Money
            ...
            Based on:
                ...
                In Proceedings of the 2011 Workshop on Context-awareness in
                Retrieval and Recommendation (CaRR '11)
        """
...
class UserProfileInversePopularity(BaseUserProfile):
    """ User Profile formed by the Weighted Sum of:
            (Event Vector Representations) x (1/log(|Event RSVPs| + 2, 2))
            The intuition behind it is that the event importance to the user is inversely
            proportional to the event size.
            Based on the results of the Content-Based Recommender (a baseline model)
            "Item Cold-Start Recommendations: Learning Local Collective Embeddings" (RecSys 2014)
    """
